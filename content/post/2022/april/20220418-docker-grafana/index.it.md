+++
date = "2022-04-18"
title = "Grandi cose con i contenitori: Grafana su Synology - Diskstation"
difficulty = "level-3"
tags = ["Monitoring", "Grafana", "Docker", "docker-compose", "Synology", "influxdb", "telegraf"]
githublink = "https://github.com/terrorist-squad/knedelverse/blob/master/content/post/2022/april/20220418-docker-grafana/index.it.md"
+++
Grafana è un'applicazione open source multipiattaforma per visualizzare graficamente i dati da varie fonti di dati come InfluxDB, MySQL, PostgreSQL o Prometheus. Oggi mostro come installare un servizio Grafana sulla Synology DiskStation.
...
## Passo 2: creare la cartella Grafana
Creo una nuova directory chiamata "grafana" nella directory Docker.
{{< gallery match="images/3/*.png" >}}
Poi creo un file "grafana.yml" nella directory di Grafana con questo contenuto:
```
version: "2"
services:
  grafana:
...
```
Creo altre due sottocartelle per i dati:
{{< terminal >}}
mkdir grafana
mkdir influxdb
...
{{</ terminal >}}
Poiché voglio usare SSL per Grafana e l'interfaccia associata, un certificato deve ancora essere creato:
{{< terminal >}}
mkdir ssl
...
Infine, creo un file "telegraf.conf" con questo contenuto:
```
[global_tags]
...
```
Ora avvia il server Grafana con questa chiamata Compose:
{{< terminal >}}
...
Ora posso chiamare il server, configurare il database, creare la dashboard e collegare altri dati di misurazione.

+++
date = "2022-04-18"
title = "容器的伟大之处：Synology 上的 Grafana - Diskstation"
difficulty = "level-3"
tags = ["Monitoring", "Grafana", "Docker", "docker-compose", "Synology", "influxdb", "telegraf"]
githublink = "https://github.com/terrorist-squad/knedelverse/blob/master/content/post/2022/april/20220418-docker-grafana/index.zh.md"
+++
Grafana是一个跨平台的开源应用程序，用于图形化显示来自各种数据源的数据，如InfluxDB、MySQL、PostgreSQL或Prometheus。今天我展示了如何在Synology DiskStation上安装一个Grafana服务。
## 第1步：准备好Synology
...
## 第2步：创建Grafana文件夹
我在Docker目录下创建了一个名为 "grafana "的新目录。
{{< gallery match="images/3/*.png" >}}
然后我在Grafana目录下创建一个 "grafana.yml "文件，其中有这样的内容。
```
version: "2"
services:
  grafana:
...
{{</ terminal >}}
由于我想为Grafana和相关界面使用SSL，所以仍然需要创建一个证书。
{{< terminal >}}
mkdir ssl
...
最后，我创建了一个有此内容的 "telegraf.conf "文件。
```
[global_tags]
...
## 第3步：配置
现在我可以调用服务器，配置数据库，创建仪表盘并连接更多的测量数据。

+++
date = "2022-04-18"
title = "Suuria asioita konttien avulla: Grafana Synologyn kanssa - Diskstation"
difficulty = "level-3"
tags = ["Monitoring", "Grafana", "Docker", "docker-compose", "Synology", "influxdb", "telegraf"]
githublink = "https://github.com/terrorist-squad/knedelverse/blob/master/content/post/2022/april/20220418-docker-grafana/index.fi.md"
+++
Grafana on alustarajat ylittävä avoimen lähdekoodin sovellus, jolla voidaan näyttää graafisesti tietoja eri tietolähteistä, kuten InfluxDB:stä, MySQL:stä, PostgreSQL:stä tai Prometheuksesta. Tänään näytän, miten Grafana-palvelu asennetaan Synology DiskStationiin.
...
## Vaihe 2: Luo Grafana-kansio
Luon Docker-hakemistoon uuden hakemiston nimeltä "grafana".
{{< gallery match="images/3/*.png" >}}
Sitten luon Grafana-hakemistoon tiedoston "grafana.yml", jossa on tämä sisältö:
```
version: "2"
services:
  grafana:
...
```
Luon kaksi muuta alikansiota tietoja varten:
{{< terminal >}}
mkdir grafana
mkdir influxdb
...
{{</ terminal >}}
Koska haluan käyttää SSL-yhteyttä Grafanassa ja siihen liittyvässä käyttöliittymässä, varmenne on vielä luotava:
{{< terminal >}}
mkdir ssl
...
Lopuksi luon tiedoston "telegraf.conf", jossa on tämä sisältö:
```
[global_tags]
...
```
Käynnistä nyt Grafana-palvelin tällä Compose-kutsulla:
{{< terminal >}}
...
Nyt voin soittaa palvelimelle, määrittää tietokannan, luoda kojelaudan ja liittää lisää mittaustietoja.
